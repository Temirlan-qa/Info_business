...
async def process_start_command(message: types.Message):
	...
	await message.reply("/start - start\n/post - юзерлерге жиберед\n/poll - опрос\n/getusers - барлык юзерлерди кору\n/hour - вебинар бастау", reply_markup=markup)
...
async def get_answer(update):
	...
	for answer in answers:
		if answer.voter_count == 1:
			ret = answer.text
	return ret
	await bot.send_poll(chat_id=GROUP_ID, question=poll.question,options=poll.options, type="quiz", correct_option_id=1, is_anonymous=True)
